            ...
            if args.viz:
                with torch.no_grad():
                    ...
                    if isinstance(model, LatentODE) and (
                            args.dataset == "periodic"):  # and not args.classic_rnn and not args.ode_rnn:
                        ...
                        w_run.log({"trajs_prior": wandb.Image(viz.fig_traj_prior_solo)})
                        w_run.log({"field": wandb.Image(viz.fig_field_solo)})
                        w_run.log({"trajs_latent": wandb.Image(viz.fig_latent_solo)})
